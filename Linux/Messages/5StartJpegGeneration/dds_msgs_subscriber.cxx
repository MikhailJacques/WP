...
int process_data(dds::sub::DataReader<dds_msgs::StartJpegGenerationMsg> reader)
{
    // Take all samples
    int count = 0;
    dds::sub::LoanedSamples<dds_msgs::StartJpegGenerationMsg> samples = reader.take();
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::topic::Topic<dds_msgs::StartJpegGenerationMsg> topic(participant, "StartJpegGenerationMsg");

    // Create a Subscriber and DataReader with default Qos
    dds::sub::Subscriber subscriber(participant);
    // dds::sub::DataReader<dds_msgs::StartJpegGenerationMsg> reader(subscriber, topic);
    dds::core::QosProvider m_qos_provider = dds::core::QosProvider::Default();
    dds::sub::DataReader<dds_msgs::StartJpegGenerationMsg>reader(subscriber, topic, m_qos_provider.datareader_qos("WorldPerceptionQoS::StartJpegGenerationMsg"));
    ...
    while (!application::shutdown_requested && samples_read < sample_count) {
        ...
        std::cout << "dds_msgs::StartJpegGenerationMsg subscriber sleeping up to 1 sec..." << std::endl;
