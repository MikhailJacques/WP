...
{ 
	...
	file_path << SolutionDir << "logs\\tcp_client_log_" + m_event_logger.Get_Timestamp_File() + ".txt";
	m_event_logger.Start(file_path.str(), "TCP CLIENT: Event logger is started");
}
...
void TCP_Client::Close(void)
{
	...
	{
		m_event_logger.Print("TCP CLIENT: Shutting down");
