...
int process_data(dds::sub::DataReader<dds_msgs::BaseMsg> reader)
{
    // Take all samples
    int count = 0;
    dds::sub::LoanedSamples<dds_msgs::BaseMsg> samples = reader.take();
...
void run_subscriber_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::topic::Topic<dds_msgs::BaseMsg> topic(participant, "EndMissionMsg");

    // Create a Subscriber and DataReader with default Qos
    dds::sub::Subscriber subscriber(participant);
    dds::sub::DataReader<dds_msgs::BaseMsg> reader(subscriber, topic);
    ...
    while (!application::shutdown_requested && samples_read < sample_count) {
        ...
        std::cout << "EndMissionMsg subscriber sleeping up to 1 sec..." << std::endl;
