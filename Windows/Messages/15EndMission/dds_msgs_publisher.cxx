...
void run_publisher_application(unsigned int domain_id, unsigned int sample_count)
{
    ...
    dds::pub::DataWriter<dds_msgs::BaseMsg> writer(publisher, topic, m_qos_provider.datawriter_qos("WorldPerceptionQoS::EndMissionMsg"));

    dds_msgs::BaseMsg data;
    for (unsigned int samples_written = 0; !application::shutdown_requested && samples_written < sample_count; samples_written++)
    {
        ...
        writer.write(data);
        std::cout << "Writing EndMissionMsg, count " << samples_written << std::endl;
