...
// DOCUMENT:	
// DESCRIPTION: This file implements flight path algorithm
...
int main()
{
    ...
    vector<GEO_POINT> flight_path;          // Stores navigation points around circular flight path
    ...
    {
        GEO_POINT geo_point((latitude + radius * cos(RAD2DEG(ii))), (longitude + radius * sin(RAD2DEG(ii))));
    ...
    {
        GEO_POINT geo_point((latitude - radius * cos(RAD2DEG(180.0 - ii))), (longitude + radius * sin(RAD2DEG(180.0 - ii))));
        flight_path.push_back(geo_point);
    }
    ...
    {
        GEO_POINT geo_point((latitude - radius * cos(RAD2DEG(ii - 180.0))), (longitude - radius * sin(RAD2DEG(ii - 180.0))));
        flight_path.push_back(geo_point);
    }
    ...
    {
        GEO_POINT geo_point((latitude + radius * cos(RAD2DEG(360.0 - ii))), (longitude - radius * sin(RAD2DEG(360.0 - ii))));
        flight_path.push_back(geo_point);
    }
    ...
    std::for_each(std::begin(flight_path), std::end(flight_path), [](GEO_POINT const& value) {
        std::cout << setprecision(4) << fixed << showpoint << value.m_latitude << " " << value.m_longitude << "\n";
        });
}
